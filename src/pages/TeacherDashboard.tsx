...
export default function TeacherDashboard({ user }: TeacherDashboardProps) {
  ...
  if (!user.approved) {
    return (
      <div className="flex flex-col items-center justify-center p-20 text-center bg-white rounded-[32px] border border-[#5A5A40]/10">
        <AlertCircle size={48} className="text-[#5A5A40]/40 mb-4" />
        <h2 className="text-2xl font-bold text-[#5A5A40] mb-2">Account Pending Approval</h2>
        <p className="text-[#5A5A40]/60 italic">Your teacher account is currently being reviewed by an administrator. Please check back later.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-10">
      <header className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-bold text-[#5A5A40] tracking-tight">Teacher Dashboard</h2>
          <p className="text-[#5A5A40]/60 italic">Manage your quizzes and monitor student progress.</p>
        </div>
      ...
      {activeTab === 'quizzes' ? (
        ...
        {quizzes.length === 0 && !loading ? (
          <div className="col-span-full bg-white/50 border border-dashed border-[#5A5A40]/20 rounded-[32px] py-20 text-center">
            <p className="text-[#5A5A40]/40 italic">No quizzes created yet. Start by creating one!</p>
          </div>
        ) : (
          quizzes.map((quiz) => (
            <motion.div 
            ...
            >
              ...
              <div className="space-y-2 mb-6 flex-grow">
                <div className="flex items-center gap-2 text-sm text-[#5A5A40]/60">
                  <span className="font-bold text-[#5A5A40]/40 uppercase tracking-widest text-[10px]">Code:</span>
                  <span className="font-mono bg-[#5A5A40]/5 px-2 py-0.5 rounded">{quiz.quizCode}</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-[#5A5A40]/60">
                  <span className="font-bold text-[#5A5A40]/40 uppercase tracking-widest text-[10px]">Duration:</span>
                  <span>{quiz.duration} mins</span>
                </div>
                {quiz.sectionId && (
                  <div className="flex items-center gap-2 text-sm text-[#5A5A40]/60">
                    <span className="font-bold text-[#5A5A40]/40 uppercase tracking-widest text-[10px]">Section:</span>
                    <span className="font-bold text-[#5A5A40]">{sections.find(s => s.id === quiz.sectionId)?.name || 'Unknown'}</span>
                  </div>
                )}
                {quiz.department && (
                  <div className="flex items-center gap-2 text-sm text-[#5A5A40]/60">
                    <span className="font-bold text-[#5A5A40]/40 uppercase tracking-widest text-[10px]">Dept:</span>
                    <span>{quiz.department}</span>
                  </div>
                )}
                <div className="flex items-center gap-2 text-sm text-[#5A5A40]/60">
                  <span className="font-bold text-[#5A5A40]/40 uppercase tracking-widest text-[10px]">Attempts:</span>
                  <span className="font-bold text-blue-600">{submissionCounts[quiz.id] || 0}</span>
                </div>
              </div>

              <div className="pt-4 border-t border-[#5A5A40]/5 flex items-center gap-2">
                <button 
                  onClick={() => togglePublish(quiz)}
                  className="p-2 hover:bg-[#5A5A40]/5 rounded-full text-[#5A5A40]/60 transition-colors"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {sections.length === 0 ? (
            <div className="col-span-full bg-white/50 border border-dashed border-[#5A5A40]/20 rounded-[32px] py-20 text-center">
              <p className="text-[#5A5A40]/40 italic">No sections created yet. Start by creating one!</p>
            </div>
          ) : (
            sections.map((section) => (
              ...
              >
                ...
                <div className="mt-auto pt-4 border-t border-[#5A5A40]/5 flex items-center justify-between">
                  <button 
                    onClick={() => { setSelectedSection(section); setShowAnnouncementModal(true); }}
                    className="text-sm font-bold text-[#5A5A40] hover:underline flex items-center gap-2"
                  >
                    <AlertCircle size={16} />
                    Manage
                  </button>
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-[48px] border border-[#5A5A40]/10 shadow-sm overflow-hidden">
            ...
            <div className="pt-16 pb-10 px-10 text-center">
              ...
              <p className="text-[#5A5A40]/40 uppercase tracking-widest text-[10px] font-bold mb-8">@{user.username} • Teacher</p>
              ...
              <div className="mt-10 pt-8 border-t border-[#5A5A40]/5">
                <button className="px-8 py-3 bg-[#5A5A40] text-white rounded-full font-medium hover:bg-[#5A5A40]/90 transition-colors">
                  Edit Profile
                </button>
              </div>
            </div>
        ...
        {showAnalyticsModal && selectedQuiz && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div 
              ...
              className="bg-[#FDFCF8] w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-[48px] p-10 shadow-2xl relative"
            >
              <button 
              ...
              <div className="mb-10">
                <h3 className="text-3xl font-bold text-[#5A5A40] mb-2">{selectedQuiz.title} - Analytics</h3>
                <p className="text-[#5A5A40]/60 italic">Detailed performance breakdown and student attempts.</p>
              </div>
              ...
              {analytics ? (
                <div className="space-y-10">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-white p-8 rounded-[32px] border border-[#5A5A40]/10 shadow-sm">
                      <p className="text-sm font-bold text-[#5A5A40]/40 uppercase tracking-widest mb-2">Average Score</p>
                      <p className="text-4xl font-bold text-[#5A5A40]">{analytics.averageScore.toFixed(1)}%</p>
                    </div>
                    <div className="bg-white p-8 rounded-[32px] border border-[#5A5A40]/10 shadow-sm">
                      <p className="text-sm font-bold text-[#5A5A40]/40 uppercase tracking-widest mb-2">Pass Rate</p>
                      <p className="text-4xl font-bold text-green-600">{analytics.passRate.toFixed(1)}%</p>
                    </div>
                    <div className="bg-white p-8 rounded-[32px] border border-[#5A5A40]/10 shadow-sm">
                      <p className="text-sm font-bold text-[#5A5A40]/40 uppercase tracking-widest mb-2">Total Attempts</p>
                  ...
                  {analytics.difficultQuestions.length > 0 && (
                    <div className="bg-white p-8 rounded-[32px] border border-[#5A5A40]/10 shadow-sm">
                      <h4 className="text-xl font-bold text-[#5A5A40] mb-6 flex items-center gap-2">
                        ...
                        {analytics.difficultQuestions.map((q, i) => (
                          <div key={q.questionId} className="flex justify-between items-center p-4 bg-red-50/50 rounded-2xl border border-red-100/50">
                            <div className="flex-1">
                              <p className="text-sm font-bold text-[#5A5A40]">{i + 1}. {q.text}</p>
                              <p className="text-xs text-[#5A5A40]/40 uppercase tracking-widest mt-1">Fail Rate: {q.failRate.toFixed(1)}%</p>
                            </div>
                  ...
                  <div className="bg-white rounded-[32px] border border-[#5A5A40]/10 overflow-hidden shadow-sm">
                    <div className="p-8 border-b border-[#5A5A40]/10 flex justify-between items-center">
                      <h4 className="text-xl font-bold text-[#5A5A40]">Student Attempts</h4>
                      <button className="text-[#5A5A40]/60 hover:text-[#5A5A40] flex items-center gap-2 text-sm font-bold">
                        <Download size={18} />
                        Export CSV
                      </button>
                    </div>
                      ...
                      <table className="w-full text-left border-collapse">
                        <thead>
                          <tr className="bg-[#5A5A40]/5 border-b border-[#5A5A40]/10">
                            <th className="px-8 py-4 text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Student Name</th>
                            <th className="px-8 py-4 text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Score</th>
                            <th className="px-8 py-4 text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Violations</th>
                            <th className="px-8 py-4 text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Time</th>
                            <th className="px-8 py-4 text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60 text-right">Action</th>
                          </tr>
                        </thead>
                        <tbody>
                          {submissions.map((sub) => (
                            <tr key={sub.id} className="border-b border-[#5A5A40]/5 hover:bg-[#5A5A40]/2 transition-colors">
                              <td className="px-8 py-6">
                                <p className="font-bold text-[#5A5A40]">{sub.studentName}</p>
                              </td>
                              <td className="px-8 py-6">
                              ...
                              <td className="px-8 py-6 text-right">
                                <button className="text-[#5A5A40]/40 hover:text-[#5A5A40]">
                ...
                <div className="py-20 text-center bg-white/50 border border-dashed border-[#5A5A40]/20 rounded-[32px]">
                  <p className="text-[#5A5A40]/40 italic">No submissions yet for this quiz.</p>
                </div>
              )}
            </motion.div>
          </div>
        ...
        {showSectionModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/20 backdrop-blur-sm">
            ...
            >
              ...
              <h3 className="text-2xl font-bold text-[#5A5A40] mb-8">
                {editingSection?.id ? 'Edit Section' : 'Create New Section'}
              </h3>

              <form onSubmit={handleCreateSection} className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Section Name</label>
                  <input 
                    type="text" 
                    required
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Description</label>
                  <textarea 
                    value={editingSection?.description || ''}
                    onChange={(e) => setEditingSection({ ...editingSection, description: e.target.value })}
                    className="w-full px-4 py-3 rounded-2xl border border-[#5A5A40]/20 focus:outline-none focus:border-[#5A5A40] transition-all h-24 resize-none"
        ...
        {showAnnouncementModal && selectedSection && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/20 backdrop-blur-sm">
            ...
            >
              ...
              <h3 className="text-2xl font-bold text-[#5A5A40] mb-2">Manage Section: {selectedSection.name}</h3>
              <p className="text-[#5A5A40]/60 italic mb-8">Post announcements and manage students.</p>
              ...
              <div className="flex-grow overflow-y-auto space-y-10 pr-4 custom-scrollbar">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                  <div className="space-y-6">
                    <h4 className="font-bold text-[#5A5A40] flex items-center gap-2">
                      <AlertCircle size={18} className="text-[#5A5A40]/40" />
                      Post New Announcement
                    </h4>
                    <form onSubmit={handlePostAnnouncement} className="space-y-4 bg-[#5A5A40]/2 p-6 rounded-3xl border border-[#5A5A40]/5">
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Title</label>
                        <input 
                          type="text" 
                          required
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Type</label>
                        <select 
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Content</label>
                        <textarea 
                          required
                          value={newAnnouncement.content || ''}
                          onChange={(e) => setNewAnnouncement({ ...newAnnouncement, content: e.target.value })}
                          className="w-full px-4 py-3 rounded-2xl border border-[#5A5A40]/20 focus:outline-none focus:border-[#5A5A40] transition-all h-32 resize-none"
                          placeholder="Write your message here..."
                        />
                      </div>
                      <button 
                        type="submit"
                        className="w-full py-3 bg-[#5A5A40] text-white rounded-full font-medium hover:bg-[#5A5A40]/90 transition-colors"
                      >
                        Post Announcement
                      </button>
                    </form>
                  ...
                  <div className="space-y-6">
                    <h4 className="font-bold text-[#5A5A40] flex items-center gap-2">
                      <Users size={18} className="text-[#5A5A40]/40" />
                      Enrolled Students ({enrollments.length})
                    </h4>
                    <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                      {enrollments.map((enroll) => (
                        <div key={enroll.id} className="flex items-center justify-between p-4 bg-white border border-[#5A5A40]/10 rounded-2xl">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-[#5A5A40]/5 flex items-center justify-center text-[#5A5A40]/40">
                              <User size={16} />
                            </div>
                            <div>
                              <p className="font-bold text-[#5A5A40] text-sm">{enroll.studentName}</p>
                              <p className="text-[10px] text-[#5A5A40]/40 uppercase tracking-widest">Enrolled {new Date(enroll.enrolledAt).toLocaleDateString()}</p>
                            </div>
                          </div>
                          <button 
                            onClick={async () => {
                      ...
                      {enrollments.length === 0 && (
                        <p className="text-[#5A5A40]/40 italic text-center py-10">No students enrolled yet.</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <h4 className="font-bold text-[#5A5A40]">Recent Announcements</h4>
                  <div className="space-y-4">
                    {announcements.map((announce) => (
                      <div key={announce.id} className={`p-6 rounded-3xl border ${announce.type === 'exam' ? 'bg-red-50/30 border-red-100' : 'bg-white border-[#5A5A40]/10'}`}>
                        <div className="flex justify-between items-start mb-2">
                          <h5 className="font-bold text-[#5A5A40]">{announce.title}</h5>
                          <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${announce.type === 'exam' ? 'bg-red-100 text-red-700' : 'bg-[#5A5A40]/10 text-[#5A5A40]'}`}>
                            {announce.type}
                          </span>
                        </div>
                        <p className="text-sm text-[#5A5A40]/70 mb-4">{announce.content}</p>
                        <div className="flex justify-between items-center text-[10px] text-[#5A5A40]/40 font-bold uppercase tracking-widest">
                    ...
                    {announcements.length === 0 && (
                      <p className="text-[#5A5A40]/40 italic text-center py-10">No announcements posted yet.</p>
                    )}
                  </div>
                </div>
              </div>
            ...
            >
              ...
              <form onSubmit={handleCreateQuiz} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Quiz Title</label>
                    <input 
                      type="text" 
                      required
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Section (Optional)</label>
                    <select 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Department / Category</label>
                    <input 
                      type="text" 
                      required
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Duration (Minutes)</label>
                    <input 
                      type="number" 
                      required
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Quiz Code (Optional)</label>
                    <input 
                      type="text" 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Start Time</label>
                    <input 
                      type="datetime-local" 
                      required
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">End Time</label>
                    <input 
                      type="datetime-local" 
                      required
        ...
        {showQuestionModal && selectedQuiz && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/20 backdrop-blur-sm">
            ...
            >
              ...
              <h3 className="text-2xl font-bold text-[#5A5A40] mb-2">Manage Questions</h3>
              <p className="text-[#5A5A40]/60 italic mb-8">{selectedQuiz.title}</p>

              <div className="flex-grow overflow-y-auto space-y-6 pr-4 custom-scrollbar">
                <div className="bg-[#5A5A40]/5 p-6 rounded-[24px]">
                  <h4 className="font-bold text-[#5A5A40] mb-4">{editingQuestion?.id ? 'Edit Question' : 'Add New Question'}</h4>
                  <form onSubmit={handleSaveQuestion} className="space-y-4">
                    <div className="space-y-2">
                      <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Question Text</label>
                      <input 
                        type="text" 
                        required
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Points</label>
                        <input 
                          type="number" 
                          required
                    ...
                    {editingQuestion?.type === 'mcq' && (
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Options (Comma separated)</label>
                        <input 
                          type="text" 
                          required
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Correct Answer</label>
                        <input 
                          type="text" 
                          required
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-[#5A5A40]/60">Explanation (Optional)</label>
                        <input 
                          type="text" 
                    ...
                    <div className="flex justify-end gap-3">
                      {editingQuestion && (
                        <button 
                          ...
                          className="px-6 py-2 rounded-full border border-[#5A5A40]/20 text-[#5A5A40] font-medium hover:bg-[#5A5A40]/5 transition-colors"
                        >
                          Cancel
                        </button>
                      )}
                      <button 
                        type="submit"
                        className="px-8 py-2 bg-[#5A5A40] text-white rounded-full font-medium hover:bg-[#5A5A40]/90 transition-colors flex items-center gap-2"
                      >
                        <Save size={16} />
                ...
                <div className="space-y-4">
                  <h4 className="font-bold text-[#5A5A40]">Questions ({questions.length})</h4>
                  {questions.map((q, i) => (
                    <div key={q.id} className="p-4 border border-[#5A5A40]/10 rounded-2xl flex justify-between items-center">
                      <div>
                        <p className="font-bold text-[#5A5A40]">{i + 1}. {q.text}</p>
                        <p className="text-xs text-[#5A5A40]/40 uppercase tracking-widest">{q.type} • {q.points} Points</p>
                      </div>
                      <div className="flex gap-2">
                        <button 
